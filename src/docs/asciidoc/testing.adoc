...
The Spring MVC com.lvlifeng.DemoTests framework builds on the mock Servlet API objects to provide an
...
<<spring-mvc-test-framework, Spring MVC com.lvlifeng.DemoTests Framework>> instead.
...
com.lvlifeng.DemoTests classes typically declare either an array of resource locations for XML or Groovy
...
[[testing-fixture-di]]
==== Dependency Injection of com.lvlifeng.DemoTests Fixtures
...
com.lvlifeng.DemoTests property sources have higher precedence than those loaded from the operating
...
Spring's `@Timed` annotation has different semantics than JUnit 4's `@com.lvlifeng.DemoTests(timeout=...)`
support. Specifically, due to the manner in which JUnit 4 handles test execution timeouts
(that is, by executing the test method in a separate `Thread`), `@com.lvlifeng.DemoTests(timeout=...)`
...
Spring Boot com.lvlifeng.DemoTests.
...
Since JUnit Jupiter supports the use of `@com.lvlifeng.DemoTests`, `@RepeatedTest`, `ParameterizedTest`,
...
the `@com.lvlifeng.DemoTests` and `@Tag` annotations from JUnit Jupiter with the `@Transactional`
...
----
	...
	@com.lvlifeng.DemoTests // org.junit.jupiter.api.com.lvlifeng.DemoTests
...
* com.lvlifeng.DemoTests instance post-processing.
...
==== com.lvlifeng.DemoTests Execution Events
...
for which it is responsible. com.lvlifeng.DemoTests instances do not automatically receive access to the
...
com.lvlifeng.DemoTests classes that use the TestContext framework do not need to extend any particular
...
[[testcontext-ctx-management-property-sources]]
===== Context Configuration with com.lvlifeng.DemoTests Property Sources
...
====== Declaring com.lvlifeng.DemoTests Property Sources
...
====== Inheriting and Overriding com.lvlifeng.DemoTests Property Sources
...
.com.lvlifeng.DemoTests suites and forked processes
[NOTE]
====
...
===== com.lvlifeng.DemoTests-managed Transactions

com.lvlifeng.DemoTests-managed transactions are transactions that are managed declaratively by using the
...
* JUnit 4's `@com.lvlifeng.DemoTests(timeout = ...)` support and `TimeOut` rule
* JUnit Jupiter's `assertTimeoutPreemptively(...)` methods in the
  `org.junit.jupiter.api.Assertions` class
* TestNG's `@com.lvlifeng.DemoTests(timeOut = ...)` support
====
...
hierarchy runs within a transaction. com.lvlifeng.DemoTests methods that are not annotated with
...
----
	...
	@Transactional
	@com.lvlifeng.DemoTests // no expected exception!
	...
	@Transactional
	@com.lvlifeng.DemoTests(expected = ...)
...
==== Parallel com.lvlifeng.DemoTests Execution
...
`@BeforeAll`, `@AfterAll`, `@BeforeEach`, `@AfterEach`, `@com.lvlifeng.DemoTests`, `@RepeatedTest`,
...
[[spring-mvc-test-framework]]
=== Spring MVC com.lvlifeng.DemoTests Framework

The Spring MVC com.lvlifeng.DemoTests framework provides first class support for testing Spring MVC code
...
Spring MVC com.lvlifeng.DemoTests also provides client-side support for testing code that uses the
...
The goal of Spring MVC com.lvlifeng.DemoTests is to provide an effective way to test controllers by
performing requests and generating responses through the actual `DispatcherServlet`.

Spring MVC com.lvlifeng.DemoTests builds on the familiar <<mock-objects-servlet, "`mock`" implementations of
...
Jupiter-based example uses Spring MVC com.lvlifeng.DemoTests:
...
In Spring MVC com.lvlifeng.DemoTests, async requests can be tested by asserting the produced async value
...
There are no options built into Spring MVC com.lvlifeng.DemoTests for container-less testing of streaming
...
===== Spring MVC com.lvlifeng.DemoTests vs End-to-End Tests

Spring MVC com.lvlifeng.DemoTests is built on Servlet API mock implementations from the
...
There are pros and cons for each approach. The options provided in Spring MVC com.lvlifeng.DemoTests are
different stops on the scale from classic unit testing to full integration testing. To be
certain, none of the options in Spring MVC com.lvlifeng.DemoTests fall under the category of classic unit
...
Another important distinction when using Spring MVC com.lvlifeng.DemoTests is that, conceptually, such
...
sample tests] intended to show how to use Spring MVC com.lvlifeng.DemoTests. You can browse these examples
for further ideas. Also, the
https://github.com/spring-projects/spring-mvc-showcase[`spring-mvc-showcase`] project has
full test coverage based on Spring MVC com.lvlifeng.DemoTests.
...
* com.lvlifeng.DemoTests JavaScript within pages.
...
With Spring MVC com.lvlifeng.DemoTests, we can easily test if we are able to create a `Message`, as follows:
...
To use Selenium WebDriver with the Spring MVC com.lvlifeng.DemoTests framework, make sure that your project
...
----
	...
	// com.lvlifeng.DemoTests code that uses the above RestTemplate ...
...
Spring MVC com.lvlifeng.DemoTests's own tests include
...
* https://mockito.github.io[Mockito]: Java mock library based on the
  http://xunitpatterns.com/com.lvlifeng.DemoTests%20Spy.html[com.lvlifeng.DemoTests Spy] pattern. Used by the Spring Framework
  in its test suite.
